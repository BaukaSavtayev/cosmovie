...
let url = '/engine/ajax/filminfo/moreinfo.php'
function getMoreInfo(id, elem){
    fetch('/engine/ajax/controller.php?mod=moreinfo&id=' + id)
        .then((response) => {
        ...
        .then((data) => {
            ...
            moreInfo.innerHTML = data
            if (elem.getBoundingClientRect().left < window.innerWidth -
                elem.getBoundingClientRect().right) moreInfo.style.left = '187px'
            else moreInfo.style.right = '187px'
            elem.appendChild(moreInfo)
        })
        .catch(() => {
            console.log('error');
        })
    ...
    // }
    // xhttp.open("GET", '/engine/ajax/controller.php?mod=moreinfo&id=' + 4, true)
    // xhttp.send()
...
for (const ajaxInfoBtn of ajaxInfoBtns) {
    ajaxInfoBtn.onmouseover = function (e) {
        ...
        let targetElemAttr = e.target.getAttribute('data-id')
        if (!InfoBtns.includes(targetElemAttr)) {
            targetElem.classList.add("loading");
            InfoBtns.push(targetElemAttr)
            getMoreInfo(targetElemAttr, targetElem.parentElement)
            setTimeout(() => e.target.classList.remove("loading"), 500)
        }

    }
    ajaxInfoBtn.onclick = (e) => e.stopPropagation()
}
//
// function getAjaxInfo(targetElem) {
//     return targetElem + ' Ajax ������ ���������� � ������';
// }
...
let addfavors = document.querySelectorAll('.addfavor')
addfavors.forEach((elem) => {
    elem.onclick = (e) => {
...
const msFormLogin = document.querySelector('#ms_form_login')
if (msFormLogin){
    msFormLogin.addEventListener('click', (e) => {
        document.querySelector('.ms_form_close').click()
        showLoginForm()
        e.preventDefault()
    })
}
