...
if( $use_banners AND $config['allow_banner'] AND count( $banners ) AND isset( $ban_short ) ) {
			...
			if ( $value[6] AND !empty( $xfieldsdata[$value[0]] ) ) {
				...
				foreach ($temp_array as $value2) {
					...
					if($value2) {
						...
						if( $config['allow_alt_url'] ) $value3[] = "<a href=\"" . $config['http_home_url'] .$value[0]."/". rawurlencode( $value4 ) . "/\">" . $value2 . "</a>";
